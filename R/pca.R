...
do.pca = function(counts, meta, file, gene_sel=NULL, lib_sel=NULL,
                  repel_annot=TRUE, legend=TRUE,
                  div=10000, annot_set=NULL, annot_cor=NULL, 
    ...
    # div: The number by which the PC values are divided
    ...
    scores=data.frame(groups, pca$x[,1:3] / div)
    ...
    geom_point(aes(color=factor(groups[,col_group_choice]),
               ...
               show.legend=legend) +
    labs(x=paste('PC1 (/', div,')', sep=""), y=paste('PC2 (/', div,')', sep="")) + 
    theme_classic() +
